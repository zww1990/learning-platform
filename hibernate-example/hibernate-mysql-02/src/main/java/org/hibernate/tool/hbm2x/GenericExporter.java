...
public class GenericExporter extends AbstractExporter {
	...
	static {
		modelIterators.put("configuration", ge -> {
			TemplateProducer producer = new TemplateProducer(ge.getTemplateHelper(), ge.getArtifactCollector());
			producer.produce(new HashMap<String, Object>(), ge.getTemplateName(),
					new File(ge.getOutputDirectory(), ge.filePattern), ge.templateName, "Configuration");
		});
		modelIterators.put("entity", ge -> {
			Iterator<?> iterator = ge.getCfg2JavaTool()
		...
		modelIterators.put("component", ge -> {
