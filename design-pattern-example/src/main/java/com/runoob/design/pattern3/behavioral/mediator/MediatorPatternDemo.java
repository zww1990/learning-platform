package com.runoob.design.pattern3.behavioral.mediator;

/**
 * @author Alienware <br>
 *         中介者模式（Mediator
 *         Pattern）是用来降低多个对象和类之间的通信复杂性。这种模式提供了一个中介类，该类通常处理不同类之间的通信
 *         ，并支持松耦合，使代码易于维护。中介者模式属于行为型模式。<br>
 * <br>
 ...
 *         关键代码：对象 Colleague 之间的通信封装到一个类中单独处理。 <br>
 *         应用实例： <br>
 ...
 *         2、各个类之间的解耦。<br>
 *         3、符合迪米特原则。 <br>
 *         缺点：中介者会庞大，变得复杂难以维护。 <br>
 *         使用场景： <br>
 *         1、系统中对象之间存在比较复杂的引用关系，导致它们之间的依赖关系结构混乱而且难以复用该对象。<br>
 *         2、想通过一个中间类来封装多个类中的行为，而又不想生成太多的子类。 <br>
 *         注意事项：不应当在职责混乱的时候使用。<br>
 */
public class MediatorPatternDemo {
	/**
	 * 我们通过聊天室实例来演示中介者模式。实例中，多个用户可以向聊天室发送消息，聊天室向所有的用户显示消息。我们将创建两个类 ChatRoom 和
	 * User。User 对象使用 ChatRoom 方法来分享他们的消息。 MediatorPatternDemo，我们的演示类使用 User
	 * 对象来显示他们之间的通信。
	 */
