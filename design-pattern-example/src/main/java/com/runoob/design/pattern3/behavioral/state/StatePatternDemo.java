package com.runoob.design.pattern3.behavioral.state;
 ...
 *         或者是改变实例变量的值
 *         。也就是说，状态模式一般和对象的状态有关。实现类的方法有不同的功能，覆盖接口中的方法。状态模式和命令模式一样，也可以用于消除
 *         if...else 等条件选择语句。 <br>
 *         应用实例：<br>
 ...
 *         否则无法切换到新增状态，而且修改某个状态类的行为也需修改对应类的源代码。 <br>
 *         使用场景：<br>
 *         1、行为随状态改变而改变的场景。<br>
 *         2、条件、分支语句的代替者。 <br>
 *         注意事项：在行为受状态约束的时候使用状态模式，而且状态不超过 5 个。<br>
 */
