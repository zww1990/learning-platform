package com.example.schedulequartz.config;

import com.example.schedulequartz.service.exchange.GiteeHttpExchange;
import com.example.schedulequartz.service.job.GiteeJobService;
import com.example.schedulequartz.service.job.GithubJobService;
import com.example.schedulequartz.service.job.JgitJobService;
...
public class JobAutoConfiguration {
	...
	public static class JgitJobAutoConfiguration {
		...
		Trigger jgitJobTrigger(ApplicationConfig appConfig, JobDetail jgitJobDetail) {
			ApplicationConfig.JobConfig jc = appConfig.getJgitJob();
	...
	public static class GiteeJobAutoConfiguration {
		...
		Trigger giteeJobTrigger(ApplicationConfig appConfig, JobDetail giteeJobDetail) {
			ApplicationConfig.JobConfig jc = appConfig.getGiteeJob();
			return TriggerBuilder.newTrigger()//
	...
	public static class GithubJobAutoConfiguration {
		...
		Trigger githubTrigger(ApplicationConfig appConfig, JobDetail githubJobDetail) {
			ApplicationConfig.JobConfig jc = appConfig.getGithubJob();
			return TriggerBuilder.newTrigger()//
