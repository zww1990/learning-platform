...
export class WebsocketService {
  private readonly messageSubject: Subject<any>;          // subject对象,用于发送事件
  ...
  private serverTimeoutSubscription = null;               // 定时检测连接对象
  private reconnectFlag = false;                          // 重连
  private reconnectPeriod = 5 * 1000;                     // 重连失败,则5秒钟重连一次
  private reconnectSubscription = null;                   // 重连订阅对象
  private runTimeSubscription;                            // 记录运行连接subscription
  private runTimePeriod = 60 * 1000 * 10;                 // 记录运行连接时间
