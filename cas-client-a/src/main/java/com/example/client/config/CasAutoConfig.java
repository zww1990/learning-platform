...
import com.example.client.config.CasAutoConfig.CasCondition;
...
public class CasAutoConfig {
	...
	public AuthenticationFilter authenticationFilter(CasProperties props) {
		...
		if (StringUtils.hasText(props.getCasIgnorePatterns())) {
			filter.setIgnoreUrlPatternMatcherStrategyClass(
					new RegexUrlPatternMatcherStrategy(props.getCasIgnorePatterns()));
		}
		return filter;
	}
	...
	public static class CasCondition implements Condition {
		...
		public boolean matches(ConditionContext context, AnnotatedTypeMetadata metadata) {
			return CasEnvMode.PROD == context.getEnvironment().getProperty("cas-env-mode", CasEnvMode.class,
					CasEnvMode.PROD);
		}
	}
}
