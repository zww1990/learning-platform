...
export default {
  async setup() {
    ...
    return { state, fallbackImg, avatarImg, handleShow, store, handleDelete }
  },
  template: `
    <a-row :gutter="[8,8]">
      <a-col :span="4" v-for="card in state.cards" :key="card.id">
        <a-card hoverable>
          ...
          <template #actions>
            <i class="fa fa-eye" @click="handleShow(card)">查看({{card.videoHits}})</i>
            <i class="fa fa-arrow-circle-o-right"
               v-if="store.user.userType === 'ADMIN' && card.auditStatus === 'PENDING'">审核</i>
            <template v-if="store.user.username === card.creator">
              <i class="fa fa-edit" v-if="card.auditStatus !== 'PASSED'">编辑</i>
